...
public class AI {
    ...
    public void nextMove(final MainActivity board) {
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                setTextBoard();
                PointAndScore bestMove = minimax(textBoard, level, 0, 0); // (textBoard, level, recurse, depth)
                applyTextBoard(bestMove.matrix);
                board.check();
    ...
    private PointAndScore getResult(ArrayList<PointAndScore> listScore, String level) {
        PointAndScore result = listScore.get(0);
    ...
    private PointAndScore minimax(String[] textBoard, String level, int recurse, int depth) {
        ArrayList<String[]> children = generateSuccessor(textBoard, level);

        if(children == null || gameOver(textBoard)) {
            return new PointAndScore(textBoard, getScore(textBoard), depth);
        } else {
            ArrayList<PointAndScore> listScore = new ArrayList<>();
            for(int i = 0; i < children.size(); i++) {
                listScore.add(minimax(children.get(i), invertLevel(level), 1, depth + 1));
            }

            PointAndScore res = getResult(listScore, level);
            if(recurse == 1)
                res.setMatrix(textBoard);
            return res;
        }
    }
